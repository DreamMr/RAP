...
for max_step in {1..10..2}; do
work_dir=/mnt/data/users/wenbinwang/LVLM_results/llava_ov_ob5b_vrag_hrbench8k_benchmark_astar_rap_${max_step}/${MODEL_NAME}
PROCESSED_IMAGE_PATH=$work_dir/${dataset}/images
mkdir -p $work_dir
torchrun --nproc-per-node=$GPU --master_port 29501 run.py --data $processed_dataset --model $MODEL_NAME --judge chatgpt-0125 --work-dir $work_dir --model_path $MODEL_PATH --is_process_image --processed_image_path $PROCESSED_IMAGE_PATH --rag_model_path $RAG_MODEL_PATH --max_step $max_step
