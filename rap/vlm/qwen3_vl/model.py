...
class Qwen3VLChat(Qwen3VLPromptMixin, BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(use_custom_prompt=use_custom_prompt,debug=debug, is_process_image=is_process_image, processed_image_path=processed_image_path,max_step=max_step, bias_value=bias_value, rag_model_path=rag_model_path)
        ...
        if self.use_vllm:
            ...
            if listinstr(['omni'], model_path.lower()):
                ...
                self.model = AutoModelForImageTextToText.from_pretrained(
                    model_path, torch_dtype='auto', attn_implementation='flash_attention_2'
                )
    ...
    def init_index_yes_no(self):
        print("Yes:",self.processor.tokenizer("Yes").input_ids)
        print("No:",self.processor.tokenizer("No").input_ids)
        if len(self.processor.tokenizer("Yes").input_ids) == 1 and len(self.processor.tokenizer("No").input_ids) == 1:
            self.index_yes = self.processor.tokenizer("Yes").input_ids[0]
            self.index_no = self.processor.tokenizer("No").input_ids[0]
        else:
            assert len(self.processor.tokenizer("Yes").input_ids) == 2 and len(self.processor.tokenizer("No").input_ids) == 2
            self.index_yes = self.processor.tokenizer("Yes").input_ids[1]
            self.index_no = self.processor.tokenizer("No").input_ids[1]
    ...
    @torch.no_grad()
    def get_confidence_value(self, content: str, image_list: Image.Image):
        try:
        ...
        if isinstance(image_list, (list, tuple)):
            image_items = list(image_list)
        else:
            image_items = [image_list]

        user_content = [{'type': 'text', 'value': content}]
        for image in image_items:
            user_content.append({'type': 'image', 'value': image})

        messages = []
        if self.system_prompt is not None:
            messages.append({'role': 'system', 'content': self.system_prompt})
        messages.append({'role': 'user', 'content': self._prepare_content(user_content)})
        ...
        yes_id = getattr(self, 'index_yes', None)
        no_id = getattr(self, 'index_no', None)
        ...
        logits_yesno = outputs.logits[:, -1, [yes_id, no_id]]
